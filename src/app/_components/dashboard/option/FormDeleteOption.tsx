import { deleteOption } from "@/app/api/optionRequest";
...
function FormDeleteOption({ open, onClose, _id }: IForm) {
  const dispatch = useAppDispatch();
  const { toast } = useToast();

  const handleDelete = async () => {
    try {
      _id && (await dispatch(deleteOption(_id)).unwrap());
      toast({
        variant: "success",
        title: "Success",
...
export default FormDeleteOption;
