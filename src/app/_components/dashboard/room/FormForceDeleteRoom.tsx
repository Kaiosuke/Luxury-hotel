import { forceDeleteRoom } from "@/app/api/roomRequest";
...
function FormForceDeleteRoom({ open, onClose, _id }: IForm) {
  const dispatch = useAppDispatch();
  const { toast } = useToast();

  const handleDelete = async () => {
    try {
      _id && (await dispatch(forceDeleteRoom(_id)).unwrap());
      toast({
        variant: "success",
        title: "Success",
...
export default FormForceDeleteRoom;
