import { forceDeleteRoomType } from "@/app/api/roomTypeRequest";
...
function FormForceDeleteRoomType({ open, onClose, _id }: IForm) {
  const dispatch = useAppDispatch();
  const { toast } = useToast();

  const handleDelete = async () => {
    try {
      _id && (await dispatch(forceDeleteRoomType(_id)).unwrap());
      toast({
        variant: "success",
        title: "Success",
...
export default FormForceDeleteRoomType;
