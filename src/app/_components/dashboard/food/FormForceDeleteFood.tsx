import { forceDeleteFood } from "@/app/api/foodRequest";
...
function FormForceDeleteFood({ open, onClose, _id }: IForm) {
  const dispatch = useAppDispatch();
  const { toast } = useToast();

  const handleDelete = async () => {
    try {
      _id && (await dispatch(forceDeleteFood(_id)).unwrap());
      toast({
        variant: "success",
        title: "Success",
...
export default FormForceDeleteFood;
