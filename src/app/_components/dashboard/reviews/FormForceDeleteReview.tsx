import { forceDeleteReview } from "@/app/api/reviewRequest";
...
function FormForceDeleteReview({ open, onClose, _id }: IForm) {
  const dispatch = useAppDispatch();
  const { toast } = useToast();

  const handleDelete = async () => {
    try {
      _id && (await dispatch(forceDeleteReview(_id)).unwrap());
      toast({
        variant: "success",
        title: "Success",
...
export default FormForceDeleteReview;
