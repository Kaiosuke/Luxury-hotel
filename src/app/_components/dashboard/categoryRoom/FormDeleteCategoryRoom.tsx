import { deleteCategoryRoom } from "@/app/api/categoryRoomRequest";
...
function FormDeleteCategoryRoom({ open, onClose, _id }: IForm) {
  const dispatch = useAppDispatch();
  const { toast } = useToast();

  const handleDelete = async () => {
    try {
      _id && (await dispatch(deleteCategoryRoom(_id)).unwrap());
      toast({
        variant: "success",
        title: "Success",
...
export default FormDeleteCategoryRoom;
